...
void verified_chain_test() {
    ...
    first_block._currentBlockHash = Hash("1");
    v.push_block(first_block);
    Block b [10];
    for (size_t i = 0; i < 10; ++i) {
        b[i].coinbase_transaction = Coinbase(Hash("his_hash"), ZINA(1, 337), "Send this to Pavel");
        b[i]._currentBlockHash = Hash(std::string(static_cast<int>(i) + 2, '1'));
        b[i]._previousBlockHash = Hash(std::string(static_cast<int>(i) + 1, '1'));
        Transaction t(Hash("his_hash"), Hash("his_hash"), ZINA(1, 337), "Send this to Pavel");
        for (size_t j = 0; j < 10; ++j) {
            b[i].addTransaction(Transaction(Hash("his_hash"), Hash("his_hash"), ZINA(i, j), "Send this to Pavel"));
        }
...
void branching_chain_test() {
    BranchingChain bc;
    Block first_block;
    first_block._currentBlockHash = Hash("SHAHAHAHAHAH");
    ...
    Block second_block;
    second_block._previousBlockHash = Hash("SHAHAHAHAHAH");
    second_block._currentBlockHash = Hash("abobus");
...
void blockchain_test() {
    ...
    for (int _ = 2; _ <= 10; ++_) {
        Block new_block;
        new_block._currentBlockHash = Hash(std::to_string(_));
        new_block._previousBlockHash = Hash(std::to_string(_ - 1));
