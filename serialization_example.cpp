...
class gps_position {
    friend ostream &operator<<(ostream &os, const gps_position &gp);
...
ostream &operator<<(ostream &os, const gps_position &gp) {
...
class bus_stop {
    ...
    friend ostream &operator<<(ostream &os, const bus_stop &gp);
...
ostream &operator<<(ostream &os, const bus_stop &bs) {
...
public:
    ...
    bus_stop_corner(const gps_position &_lat, const gps_position &_long,
                    const string &_s1, const string &_s2
    ) :
...
public:
    ...
    bus_stop_destination(
            const gps_position &_lat, const gps_position &_long, const string &_name
    ) :
...
class bus_route {
    ...
    friend ostream &operator<<(ostream &os, const bus_route &br);

    typedef bus_stop *bus_stop_pointer;
    list<bus_stop_pointer> stops;
...
ostream &operator<<(ostream &os, const bus_route &br) {
    list<bus_stop *>::const_iterator it;
...
public:
    ...
    struct trip_info {
        ...
        trip_info(int _h, int _m, const string &_d) :
...
private:
    ...
    friend ostream &operator<<(ostream &os, const bus_schedule &bs);

    friend ostream &operator<<(ostream &os, const bus_schedule::trip_info &ti);

    list<pair<trip_info, bus_route *> > schedule;
...
public:
    void append(const string &_d, int _h, int _m, bus_route *_br) {
        schedule.insert(schedule.end(), make_pair(trip_info(_h, _m, _d), _br));
    }
...
ostream &operator<<(ostream &os, const bus_schedule::trip_info &ti) {
    return os << '\n' << ti.hour << ':' << ti.minute << ' ' << ti.driver << ' ';
}

ostream &operator<<(ostream &os, const bus_schedule &bs) {
    list<pair<bus_schedule::trip_info, bus_route *> >::const_iterator it;
